...
public:
	...
	void simulate(PxScene * , PxF32 timeStep)
	{
		...
		if (!mContainingActor->isSleeping())
		{
			...
			{
				PxU32 nbActors = mScene->getNbActors(PxActorTypeSelectionFlag::eRIGID_DYNAMIC);
				if(nbActors)
				{
					std::vector<PxRigidActor*> actors(nbActors);
					mScene->getActors(PxActorTypeSelectionFlag::eRIGID_DYNAMIC, (PxActor**)&actors[0], nbActors);
