	...
	SampleArray<PxVec2>& texcoords1, SampleArray<PxVec2>& texcoords2, SampleArray<PxVec2>& texcoords)
{
	...
	vertices.resize(numVertices);
	shdfnd::fastMemcpy(vertices.begin(), desc1.points.data, sizeof(PxVec4) * desc1.points.count);
	shdfnd::fastMemcpy(vertices.begin()+desc1.points.count, desc2.points.data, sizeof(PxVec4) * desc2.points.count);
	...
	shdfnd::fastMemcpy(indexBase, desc1.quads.data, sizeof(PxU32) * desc1.quads.count * 4);
	indexBase += desc1.quads.count * 4;

	shdfnd::fastMemcpy(indexBase, desc2.quads.data, sizeof(PxU32) * desc2.quads.count * 4);
	for (PxU32 i = 0; i < desc2.quads.count * 4; i++)
		indexBase[i] += desc1.points.count;
	indexBase += desc2.quads.count * 4;

	shdfnd::fastMemcpy(indexBase, desc1.triangles.data, sizeof(PxU32) * desc1.triangles.count * 3);
	indexBase += desc1.triangles.count * 3;
	
	shdfnd::fastMemcpy(indexBase, desc2.triangles.data, sizeof(PxU32) * desc2.triangles.count * 3);
	for (PxU32 i = 0; i < desc2.triangles.count * 3; i++)
		indexBase[i] += desc1.points.count;

	texcoords.resize(desc1.points.count + desc2.points.count);
	shdfnd::fastMemcpy(texcoords.begin(), texcoords1.begin(), sizeof(PxVec2) * desc1.points.count);
	shdfnd::fastMemcpy(texcoords.begin()+desc1.points.count, texcoords2.begin(), sizeof(PxVec2) * desc2.points.count);
	...
	SampleArray<PxVec4>& vertices, SampleArray<PxU32>& indices, SampleArray<PxVec2>& texcoords)
{
	...
	PxU32* indexBase = indices.begin();
	shdfnd::fastMemcpy(indexBase, inMesh.quads.data, sizeof(PxU32) * inMesh.quads.count * 4);
	indexBase += inMesh.quads.count * 4;
	shdfnd::fastMemcpy(indexBase, inMesh.triangles.data, sizeof(PxU32) * inMesh.triangles.count * 3);
	...
	SampleArray<PxVec4>& vertices, SampleArray<PxU32>& indices, SampleArray<PxVec2>& texcoords)
{
	...
	physx::shdfnd::fastMemcpy((PxU32*)indices.begin(), 
		wo.mIndices, sizeof(PxU32)*numTriangles*3);

	texcoords.resize(numVertices);
	physx::shdfnd::fastMemcpy(&texcoords.begin()->x, 
