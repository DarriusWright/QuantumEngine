...
{
	ProfileZone p("RenderClothActor::updateRenderShape", mRenderer.getProfileZone());
    ...
    if(!mClothRenderShape)
    {
		...
		if (dispatcher && (ctxMgr = dispatcher->getCudaContextManager()) && 
			ctxMgr->getInteropMode() != PxCudaInteropMode::D3D9_INTEROP &&
	...
	{
		ProfileZone p("RenderClothShape::update", mRenderer.getProfileZone());
		...
		if (mCloth.getClothFlags()&PxClothFlag::eGPU && mClothRenderShape->isInteropEnabled())
		{
			...
			{
				ProfileZone p("updateInterop", mRenderer.getProfileZone());
		...
		if (needUpdate)
		{
...

			{
				...
				ProfileZone p("BuildSmoothNormals", mRenderer.getProfileZone());
...
{
	ProfileZone p("RenderClothActor::update", mRenderer.getProfileZone());
	...
	{
		ProfileZone p("mCloth.getCollisionData", mRenderer.getProfileZone());
	...
	{
		ProfileZone p("updateRenderSpheres", mRenderer.getProfileZone());
	...
	{
		ProfileZone p("updateRenderCapsules", mRenderer.getProfileZone());
