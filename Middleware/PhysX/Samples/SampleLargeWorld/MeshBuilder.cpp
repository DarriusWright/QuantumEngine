										...
										const char*				name)
{
	...
	PxShape* shape = actor->createShape(PxTriangleMeshGeometry(mesh, meshScale), material);
	PX_ASSERT( shape );
										...
										const char*			name)
{
	...
	for(PxU32 i = 0; i < size; ++i)
	{
		...
		PxShape* shape = actor->createShape(PxConvexMeshGeometry(mesh, meshScale), mMaterial);
		PX_ASSERT( shape );
		shape->setFlag(PxShapeFlag::eVISUALIZATION, false);
	}
	...
	for(PxU32 i = 0; i < size; ++i)
	{
		...
		PxShape* shape = actor->createShape(PxTriangleMeshGeometry(mesh, meshScale), mMaterial);
		PX_ASSERT( shape );
		shape->setFlag(PxShapeFlag::eVISUALIZATION, false);
	}
