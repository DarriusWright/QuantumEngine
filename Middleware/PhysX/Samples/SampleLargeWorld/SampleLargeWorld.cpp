...
void SampleLargeWorld::collectInputEvents(std::vector<const SampleFramework::InputEvent*>& inputEvents)
{
	...
	DIGITAL_INPUT_EVENT_DEF(CAMERA_SPEED_INCREASE,		WKEY_ADD,			XKEY_ADD,		X1KEY_ADD,		PS3KEY_ADD,			PS4KEY_ADD,			AKEY_UNKNOWN,	OSXKEY_ADD,			PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_ADD,		WIIUKEY_UNKNOWN);
	DIGITAL_INPUT_EVENT_DEF(CAMERA_SPEED_DECREASE,		WKEY_SUBTRACT,		XKEY_SUBTRACT,	X1KEY_SUBTRACT,	PS3KEY_SUBTRACT,	PS4KEY_SUBTRACT,	AKEY_UNKNOWN,	OSXKEY_SUBTRACT,	PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_SUBTRACT,	WIIUKEY_UNKNOWN);

	DIGITAL_INPUT_EVENT_DEF(THROW_IMPORTANTOBJECT,		WKEY_I,				XKEY_I,			X1KEY_I,		PS3KEY_I,			PS4KEY_I,			AKEY_UNKNOWN,	OSXKEY_I,			PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_I,			WIIUKEY_UNKNOWN);
	DIGITAL_INPUT_EVENT_DEF(THROW_UNIMPORTANCTOBJECT,	WKEY_U,				XKEY_U,			X1KEY_U,		PS3KEY_U,			PS4KEY_U,			AKEY_UNKNOWN,	OSXKEY_U,			PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_U,			WIIUKEY_UNKNOWN);
	DIGITAL_INPUT_EVENT_DEF(PICK_NEARSETOBJECT,			WKEY_E,				XKEY_E,			X1KEY_E,		PS3KEY_E,			PS4KEY_E,			AKEY_UNKNOWN,	OSXKEY_E,			PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_E,			WIIUKEY_UNKNOWN);

	DIGITAL_INPUT_EVENT_DEF(FLY_CAMERA,					WKEY_M,				XKEY_M,			X1KEY_M,		PS3KEY_M,			PS4KEY_M,			AKEY_UNKNOWN,	OSXKEY_M,			PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_M,			WIIUKEY_UNKNOWN);
	DIGITAL_INPUT_EVENT_DEF(FLY_CAMERA,					GAMEPAD_WEST,       GAMEPAD_WEST,   GAMEPAD_WEST,   GAMEPAD_WEST,       GAMEPAD_WEST,       AKEY_UNKNOWN,	OSXKEY_UNKNOWN,		GAMEPAD_SOUTH,      IKEY_UNKNOWN,	LINUXKEY_UNKNOWN,	GAMEPAD_WEST);

	DIGITAL_INPUT_EVENT_DEF(RETRY,						WKEY_R,				XKEY_R,			X1KEY_R,		PS3KEY_R,			PS4KEY_R,			AKEY_UNKNOWN,	OSXKEY_R,			GAMEPAD_EAST,		IKEY_UNKNOWN,	LINUXKEY_R,			WIIUKEY_UNKNOWN);
	TOUCH_INPUT_EVENT_DEF(RETRY,						"Retry",		ABUTTON_1,          IBUTTON_1, TOUCH_BUTTON_1);
	TOUCH_INPUT_EVENT_DEF(FLY_CAMERA,					"Fly Mode",		ABUTTON_3,          IBUTTON_3, TOUCH_BUTTON_3);
	
	DIGITAL_INPUT_EVENT_DEF(CAMERA_SPEED_INCREASE,	GAMEPAD_RIGHT_SHOULDER_TOP,	GAMEPAD_RIGHT_SHOULDER_TOP,	GAMEPAD_RIGHT_SHOULDER_TOP,	GAMEPAD_RIGHT_SHOULDER_TOP,	GAMEPAD_RIGHT_SHOULDER_TOP,	AKEY_UNKNOWN,	GAMEPAD_RIGHT_SHOULDER_TOP,	PSP2KEY_UNKNOWN,	IKEY_UNKNOWN, LINUXKEY_UNKNOWN,	GAMEPAD_RIGHT_SHOULDER_TOP);
	DIGITAL_INPUT_EVENT_DEF(CAMERA_SPEED_DECREASE,	GAMEPAD_LEFT_SHOULDER_TOP,	GAMEPAD_LEFT_SHOULDER_TOP,	GAMEPAD_LEFT_SHOULDER_TOP,	GAMEPAD_LEFT_SHOULDER_TOP,	GAMEPAD_LEFT_SHOULDER_TOP,	AKEY_UNKNOWN,	GAMEPAD_LEFT_SHOULDER_TOP,	PSP2KEY_UNKNOWN,	IKEY_UNKNOWN, LINUXKEY_UNKNOWN,	GAMEPAD_LEFT_SHOULDER_TOP);
}
...
void SampleLargeWorld::customizeRender()
{
	...
	//if(renderSection)
	{
		...
		physx::string::sprintf_s(textBuf, 256, "Disk IO\t\t\t%0.1f ms", mDiskIOTime*1000.0f);	
		renderer->print(10, camera.getScreenHeight() - yInc*6, textBuf);

		physx::string::sprintf_s(textBuf, 256, "PhysX Streaming   %0.1f ms", mPhysxStreaming*1000.0f);	
		renderer->print(10, camera.getScreenHeight() - yInc*5, textBuf);

		physx::string::sprintf_s(textBuf, 256, "Graphic Streaming %0.1f ms", mGraphicStreaming*1000.0f);	
		renderer->print(10, camera.getScreenHeight() - yInc*4, textBuf);

		PxVec3 cameraPos = camera.getPos();
		physx::string::sprintf_s(textBuf, 256, "Position x=%0.1f,z=%0.1f", cameraPos.x, cameraPos.z);	
...
RAWMesh* SampleLargeWorld::createRawMeshFromMeshGeom(const PxTriangleMeshGeometry& mesh, RAWMesh &rawMesh)
{
	...
	const bool has16BitTriangleIndices	= tm->getTriangleMeshFlags() & PxTriangleMeshFlag::eHAS_16BIT_TRIANGLE_INDICES ? true : false;
