...
#if !defined (RENDERER_WINMODERN) && !defined (PX_XBOXONE)
#include "D3DX11.h"
#endif
...
D3DX11::D3DX11(void)
	...
	m_pD3DReflect(NULL)
{
#if defined(RENDERER_WINDOWS) && !defined(RENDERER_WINMODERN) && !defined (PX_XBOXONE)
...
D3DX11::~D3DX11(void)
{
#if defined(RENDERER_WINDOWS) && !defined(RENDERER_WINMODERN)
	if (m_library)
	{
	...
	if (errors)
	{
		...
		if (errorStr)
		{
			...
			if (!ignoreErrors)
			{
				...
				char szBuff[1024];
				sprintf(szBuff,"Shader compiler error: %s",errorStr);
