    ...
    m_renderer(renderer)
{
	...
	if (d3dDevice)
	{
		...
		for (PxU32 i = 0; i < NUM_PASSES; i++)
		{
			...
			char suffix[32];
			physx::string::sprintf_s(suffix, 32, ".cache%d", i);
...
{ 
	...
	if(cacheDir)
	{
		...
		physx::string::strcpy_s(originpath, 1024, m_renderer.getAssetDir());
		...
		physx::string::strcpy_s(relativepath, 512, shaderFilePath);
		physx::string::strcat_s(relativepath, 512, suffix);
	...
	{
		physx::shdfnd::fopen_s(&file, fullpath, "wb");		
		if(file)
		{
